#![no_std]

// 1. Re-export các định nghĩa cốt lõi từ thư viện dùng chung
pub use types::*;

pub mod traits {
    pub use hal_timer::*;
    pub use hal_interrupt::*;
    pub use hal_uart::*;
    pub use hal_display::*;
    pub use hal_arch_trait::*;
    pub use hal_paging::*;
}
pub use traits::*; // Khớp với cấu trúc hal/traits của mày

// 2. Bộ điều phối kiến trúc (The Facade)

// Hỗ trợ RV64 (Jarvis)
#[cfg(feature = "riscv64")]
pub use hal_riscv::rv64::*;
#[cfg(feature = "riscv64")]
pub use hal_riscv::common;

// Hỗ trợ RV32 (Robot Nano)
#[cfg(feature = "riscv32")]
pub use hal_riscv::rv32::*;
#[cfg(feature = "riscv32")]
pub use hal_riscv::common;

// Hỗ trợ ARM (AArch64)
#[cfg(feature = "aarch64")]
pub use hal_arm::*;

// Hỗ trợ x86_64
#[cfg(feature = "x86_64")]
pub use hal_x86::*;

// Chặn lỗi khi quên chọn mục tiêu
#[cfg(not(any(feature = "riscv64", feature = "riscv32", feature = "aarch64", feature = "x86_64")))]
compile_error!("Mày phải chọn 'riscv64', 'riscv32', 'aarch64', hoặc 'x86_64' thì ViOS mới chạy được!");
