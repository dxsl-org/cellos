...
- Each descriptor owns board identity, compatible strings, boot/firmware contract, fallback memory map, pinmux/PHY wiring, and the list of shared drivers to enable.
- The first migrated board is QEMU RV64; its fallback DTS asset is checked in for audit/reference, not claimed here as a compiled `dtc` artifact.
...
- `hal/soc/` is intentionally deferred until there is real SoC glue to extract from the current mechanism layers.
...
New board policy is kept out of HAL. The HAL owns mechanism implementations; `boards/` carries product descriptors and the kernel consumes them before boot/platform fallback decisions. Legacy AArch64/RPi3 and SDHCI board branches remain until their later migration slices.
