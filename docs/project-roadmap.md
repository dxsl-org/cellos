...
| Hot migration / zero-downtime + cell-signing mechanism | M4.1 + G.2 P2 | ✅ MECHANISMS COMPLETE 2026-06-23; Phase 00 public syscall landing is complete 2026-08-07 — `PauseService` 422 is SupervisorCap-gated with bit 49, rejects cached-TID sends and drains accepted ingress before Snapshot; `HotSwap` 400 remains, `SpawnReplacement` 421 is additive with bit 57, the exact launch-profile intersection is enforced, and refreshed-image QEMU `supervisor_hotswap_preserves_demo_state` passed. Fleet signed-only admission remains planned. | **G2** |
