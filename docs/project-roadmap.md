...
| Authenticated software evidence | Completed / regression-only | `scope-gated` | `host` | Hardware-independent roadmap Phase 07; trusted `main` run `33247922906` produced a verified attested bundle, and the fail-closed external replay store passes mutation, identity, regression, and concurrent-consumption attacks | Phase 08 may project only a freshly verified software/QEMU bundle consumed through operator-provisioned durable external state; no physical, secure-root, cloud, approval, or production claim is implied |
