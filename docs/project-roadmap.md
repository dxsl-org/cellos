...
| AWS DEV_REFERENCE nonce-bound signed time | External-gated prerequisite | `external-gated` | `host` `SOFTWARE_HARNESS` cores complete; later authorized live-service evidence | Authority Phase 5; strict canonical protocol, request/registration authentication, receipt-first persistence/recovery, allocator/clock policy, pinned KMS boundaries, handler, manifest, production rejection, non-deploying CloudFormation/IAM graph, and strict Cloudflare draft-11 provider-profile adapter pass 258 signed-time and 8 production-rejection tests; official vectors 001 and non-empty-path 010 verify, while one authenticated live response violated mandatory `NONC`/`RADI` rules and was rejected | Resolve the Cloudflare live profile compatibility failure without weakening the reviewed contract, freeze the non-restorable epoch/checkpoint authority, implement restored/fork lineage tests and package/deploy/rollback/evidence scripts, name the AWS DEV account/region, obtain deployment authorization, compose the artifact, then execute outage, restore, rollback, principal-reachability, and allocator-fork scenarios before handing the pinned manifest, vectors, and raw evidence index to Phase 6 |
