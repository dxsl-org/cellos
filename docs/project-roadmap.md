...
| Authenticated software evidence | Current executable work | `scope-gated` | `host` | Hardware-independent roadmap Phase 07; trusted `main` run `33247922906` produced a verified attested bundle binding revision, producer/toolchain identity, QEMU logs, and the exact disk-image digest | Implement consumed-sequence replay protection before Phase 08 projects the verified bundle; no physical, secure-root, cloud, approval, or production claim is implied |
