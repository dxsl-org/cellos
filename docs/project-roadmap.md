...
> truth. Midori is the sole active feature program until phases 04/07/08 close;
...
| NET_RX completion substrate | kernel/task waker + completion queue | ✅ QEMU markers PASS for completion-queue reserve/land/bound/defer, net-rx-reservation fill/remember/release, and ipc-pending deferred delivery/bounds/quota; RV64 now enables S-mode external IRQ delivery; VirtIO ACK uses scoped SUM plus exact `InterruptStatus`; NIC MMIO owner/device-type binding now points the cached NET_RX source and is the only production caller of `signal_net_rx()`; the `[net-rx-producer] irq->completion PASS` witness requires a real RX drain; shared death/hotswap clears driver roles; shell still uses `RecvTimeout` + pending mailbox delivery and `libs/ostd/src/executor.rs` remains dummy-waker busy-yield; generic reactor, peer-death CQ target-generation ABI, `RecvScatter`, and async VFS/DMA remain deferred | **G1** |
...
| Hot migration / zero-downtime + cell-signing mechanism | M4.1 + G.2 P2 | ✅ MECHANISMS COMPLETE 2026-06-23 — Ed25519 verify-at-spawn hook (default G1 still admits absent signatures; dev seed is test-only), 5-step hotswap protocol with TaskState::Frozen, ViStateTransfer trait, IPC queue preservation, HotSwapReady(401)/StateStashClear(412) syscalls, 11/11 hotswap-smoke tests (2 QEMU + 9 unit). Fleet signed-only admission remains planned. | **G2** |
...
✅ **Launch-edge deprivilege**: exact launch profiles for shell/init/hypha/tool-spawn/supervisor/pinned; `SpawnFromMem` fail-closed; launch-profile integration and boot selftest pass; init respawn proof deferred
