...
| AWS DEV_REFERENCE nonce-bound signed time | External-gated prerequisite | `external-gated` | `host` `SOFTWARE_HARNESS` protocol/provider/allocator-lineage cores complete; later authorized live-service evidence | Authority Phase 5; ADR-0011 pins the strict Cloudflare provider profile and ADR-0012 pins the external retained lineage table/key, allocator and lineage `TableId` values, canonical signed direct-child epoch transitions, restored-state epoch migration, cross-table head checks, and separate transition role. The harness passes 283 signed-time and 8 production-rejection tests. Runtime may `DescribeTable`; lineage data-plane IAM grants only transaction-enclosed `GetItem`/`ConditionCheckItem`, never mutation actions. | Resolve live provider incompatibility; freeze artifact scripts; name and authorize the AWS DEV account/region; then prove deployment, old-key disable, candidate-table rebinding, state/head CAS, restore/fork rejection, outage, rollback, and principal-reachability evidence. No current software or observed live endpoint result satisfies production admission. |
