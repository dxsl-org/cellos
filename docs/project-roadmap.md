...
| STM32 DEV_REFERENCE protected journal | External-gated prerequisite | `scope-gated` | `host` journal `SOFTWARE_HARNESS` complete; private-v2 profile transport selected; later exact-device `physical` development evidence | Authority Phase 4; canonical full-record codec, Phase 2-owned exact successor/exhaustion gate, slot-A genesis and authenticated predecessor recovery, counter-first journal, irreversible seal seam, and every-prefix cut model pass 26 journal tests plus 28 authority-protocol tests and no_std checks; authentication precedes root-profile verification; selected v2 carries the frozen 12,288-byte raw DER chain in sixteen authenticated chunks and stages a bank reference | Implement and host-verify private-v2 upload/bank recovery and the closed validator; exact STM32H573I-DK and private SLB9672 must then prove identity/NV/policy/flash limits, isolation, lifecycle/debug protection, endurance, and physical cut recovery |
