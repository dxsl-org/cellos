...
| Native CPU AI inference service | Current executable work | `ready` | `qemu` | Spec 24 CP-1..CP-3 complete: `libs/ai-proto`, `libs/ai-sdk`, `libs/gguf-rs`, `libs/ai-tokenizer`, `libs/tensor-math`, `libs/ai-engine`, `cells/services/ai` (`service::AI = 15`) | Extend the engine (SIMD kernels, quantized output projection, Q4_K quantizers, SPM tokenizers) or wire a consumer (Hypha, HTTP front end); NPU/GPU backends stay behind the G3 accelerator envelope |
