...
| VF2 DEV_REFERENCE root stream | Current executable work | `scope-gated` | `host` `SOFTWARE_HARNESS`; later exact-device `physical` development evidence | ADR-0010 and authority Phase 3; deterministic CBOR/COSE bundler, bounded shared verifier, quarantine/range/signature negatives, and linker-size work may proceed | Exact VF2 v1.3B plus STM32 authority hardware must freeze BootROM/SRAM/DRAM/address/quarantine limits and prove sole-sender/reset/strap/media negatives; host output never satisfies that gate |
