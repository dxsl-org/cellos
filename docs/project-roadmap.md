...
| Native CPU AI inference service | Current executable work | `ready` | `qemu` | Spec 24 CP-1..CP-3 plus Phases 05–06 (SentencePiece, control tokens, scaled attention, zero-copy weights, a 15M-parameter checkpoint served inside a Cell): `libs/ai-proto`, `libs/ai-sdk`, `libs/gguf-rs`, `libs/ai-tokenizer`, `libs/tensor-math`, `libs/ai-engine`, `cells/services/ai` (`service::AI = 15`) | Extend the engine (SIMD kernels, Q4_K quantizers, per-handle VIFS1 read cursor so large models load faster) or wire a consumer (Hypha, HTTP front end); NPU/GPU backends stay behind the G3 accelerator envelope |
