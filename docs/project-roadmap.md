...
| AWS DEV_REFERENCE nonce-bound signed time | External-gated prerequisite | `external-gated` | `host` `SOFTWARE_HARNESS` runtime/package/provider/allocator-lineage cores complete; later authorized live-service evidence | Authority Phase 5; ADR-0011 pins the strict Cloudflare provider profile and ADR-0012 pins the external retained lineage table/key, immutable table identities, exact transitions, restored-state migration, and cross-table checks. Cold start binds one canonical packaged manifest to the AWS region, tables, keys, lineage, and live `TableId` values; the strict payload-v2 boundary and network-free deterministic local-wheel packager pass 303 signed-time and 8 production-rejection tests. Unsigned package digests are never represented as final Lambda `CodeSha256`. | Resolve live provider incompatibility; obtain the reviewed manifest and SHA-256-indexed wheelhouse; name and authorize the AWS DEV account/region; implement signing/upload/deploy/rollback/evidence scripts; then prove old-key disable, candidate-table rebinding, state/head CAS, restore/fork rejection, outage, rollback, and principal-reachability evidence. No current local artifact or observed live endpoint result satisfies deployment or production admission. |
