...
| STM32 DEV_REFERENCE protected journal | External-gated prerequisite | `scope-gated` | `host` journal `SOFTWARE_HARNESS` complete; certificate validator protocol-blocked; later exact-device `physical` development evidence | Authority Phase 4; canonical full-record codec, Phase 2-owned exact successor/exhaustion gate, opaque internally recovered baseline, slot-A genesis invariant, authenticated counter-minus-one recovery chain, counter-first inactive-slot journal, irreversible seal seam, and every-prefix persistent cut/reboot model pass 26 journal tests plus the complete 28-test authority-protocol suite and no_std checks; authentication now precedes root-profile verification | Phase 2 protocol review must resolve required one/two-intermediate chains exceeding the frozen 768-byte profile request without truncation or policy weakening; exact STM32H573I-DK and private SLB9672 must then prove identity/NV/policy/flash limits, isolation, lifecycle/debug protection, endurance, and physical cut recovery |
