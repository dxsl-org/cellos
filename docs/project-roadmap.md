...
| Hot migration / zero-downtime + cell-signing mechanism | M4.1 + G.2 P2 | ✅ MECHANISMS COMPLETE 2026-06-23; Phase 00 public syscall landing is code-complete/runtime-unverified 2026-08-07 — `HotSwap` 400 remains, `SpawnReplacement` 421 is additive with bit 57, `SupervisorCap` gates the exact launch-profile intersection, and refreshed-image QEMU proof is still blocked by pre-existing `gen_disk` `tetris.c` / missing `app-init` plus freeze-before-snapshot ordering. Fleet signed-only admission remains planned. | **G2** |
