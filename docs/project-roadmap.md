...
| Native CPU AI inference service | Current executable work | `ready` | `qemu` | Spec 24 CP-1..CP-3 plus Phase 05 (SentencePiece, control tokens, scaled attention, a real checkpoint served inside a Cell): `libs/ai-proto`, `libs/ai-sdk`, `libs/gguf-rs`, `libs/ai-tokenizer`, `libs/tensor-math`, `libs/ai-engine`, `cells/services/ai` (`service::AI = 15`) | Extend the engine (zero-copy weight loading so a 26.7 MB story model fits a Cell, SIMD kernels, Q4_K quantizers) or wire a consumer (Hypha, HTTP front end); NPU/GPU backends stay behind the G3 accelerator envelope |
