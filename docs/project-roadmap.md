...
> `gen_disk.ps1`, and retained RV64 QEMU boot evidence from
> `phase02b-slice3-qemu-20260820-125328.log` with remote disabled and no
> panic/watchdog markers. Slice 3 re-review is PASS_WITH_RISK; production root,
> anti-rollback, broker runtime wiring, and hardware evidence remain pending.
...
| 🆕 **Cell-to-Cell Anywhere** — L.2 Internet layer (flagship feature) | *new* | ✅ Candidate B local foundation complete (P01-P03) — Phase 02A keeps remote/public exports fail-closed, while Phase 03 now has bounded broker queues, 10,000-call soak, warm baseline, clean restart/retry recovery, and a real in-cell user-thread runtime. RV64 QEMU runtime evidence and RV64/RV32/AArch64/x86_64 compile contracts pass; RV32 is compile-only without `test-hooks`, and no hardware or CI pass is claimed. Phase 02B slice 1 KMS ABI + opaque DH review passed, slice 2’s fail-closed KMS runtime scaffold is review-passed with fresh RV64 QEMU proof, and slice 3 re-review is PASS_WITH_RISK on retained RV64 QEMU evidence (one KMS start, verified registry, shell prompt, remote disabled, zero heartbeat/watchdog/panic); the `/srv/cellos` service-bound VFS code is complete, but live-volume RedoxFS persistence evidence is still pending, so Phase 04/05 remote/public work and two-node runtime verification do not start yet. See `.agents/260819-1409-cell-to-cell-anywhere-core/plan.md` | **✅ Local P01-P03 · 🧭 Phase 02A · 🧭 Phase 02B slice 1+2+3 · 📋 Remote P04-P08** |
...
- Phase 02A now loads a boot-provisioned non-secret export registry at `/etc/cellos/c2c-exports.cfg`; it can describe exported services, but it cannot authorize remote/public delivery until a separate secure node-identity authority exists. Local IPC and relay routing stay unchanged.
...
**Status**: 🔄 IN PROGRESS — implementation code-complete; strict Linux DRM/E2E and interactive T4-T9 remain externally gated on ARM64 KVM / real hardware
**Priority**: P2
...
- Verification gate: local Windows QEMU-TCG faults before the guest DRM probe; final Linux DRM/E2E evidence still needs ARM64 KVM / real hardware.
...
- ✅ **Final design**: 1 VFS service + MountTable (longest-prefix) + backend dispatch:
  BootFS (`/bin` initramfs) · RamFS (`/tmp`) · FAT32 (interop SD → `/mnt/sd`) · littlefs (`/data` power-safe, G1) · RedoxFS (`/srv`, G1 functional through VirtIO-BLK; G2 production qualification pending)
...
- [x] **Phase 2.5-5**: exFAT + Native FS — exFAT graceful fallback; RedoxFS selected for `/srv`
- [x] **D10 amendment (2026-08-01)**: retain tested RedoxFS-on-VirtIO as G1 proof-of-function; G2 qualification remains gated by automated RedoxFS-on-NVMe persistence, a defined `<100 us` read benchmark, approved purchasable hardware, and an explicit P5 authorization decision ([ADR 0002](decisions/0002-phased-srv-redoxfs-activation.md))
...
- **Measured 2026-08-01:** allocator-committed bytes = 135,782,400 (129.49 MiB), so this
  objective currently fails. The metric is now real; reducing memory is separate follow-up work.
...
✅ **RV64 boot memory discovery**: reservation-safe firmware DTB map with 2 GiB capacity gate
...
✅ **Launch-edge deprivilege**: exact launch profiles for shell/init/hypha/tool-spawn/supervisor/pinned; `SpawnFromMem` fail-closed; supervisor replacement launches stay inside reviewed target rows, the frozen-ceiling intersection, manifest/policy checks, and SupervisorCap; launch-profile integration and boot selftest pass; init respawn proof deferred
