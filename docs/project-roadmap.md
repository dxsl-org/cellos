...
| 🆕 **Cell-to-Cell Anywhere** — L.2 Internet layer (flagship feature) | *new* | ✅ G1 foundation code complete (P01-P03) + Phase 02A fail-closed registry slice complete — `net-broker` now loads `/etc/cellos/c2c-exports.cfg` as a boot-provisioned non-secret policy input, keeps remote/public disabled until secure node identity exists, and leaves Phase 02B durable identity storage blocked by the current VFS authority model. Two-node runtime verification remains pending. G2 plan forthcoming (P04 HyParView+PlumTree, P05 UDP hole-punch, P06 Pkarr/DNS, P07 K2 per-node, P08 K3 DICE). See `.agents/260624-cell-to-cell-anywhere/plan.md` | **✅ G1 (P01-P03) · 🧭 Phase 02A · 📋 G2 (P04-P08)** |
