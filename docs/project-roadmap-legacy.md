...
> **2026-08-19 ABI cleanup update:** HAL↔kernel Rust ABI signatures now live in
> `hal/traits/arch/src/kernel_abi.rs`; HAL arch code imports the shared
> declarations and the kernel keeps compile-time assertions beside the hook
> sites. The x86 page-fault hook now participates in that single-source
> contract, and the HAL boundary guard rejects new local `extern "Rust"`
> declarations. No public ABI changed.
