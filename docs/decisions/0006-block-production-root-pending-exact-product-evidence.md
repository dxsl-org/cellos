...
[ADR-0005](./0005-mutual-tls-relay-identity.md) requires external relay mTLS, and [ADR-0008](./0008-protected-relay-tls-endpoint-ownership.md) assigns the complete relay TLS endpoint to the protected authority rather than a service-net signing oracle. The authority still depends on separately selected and qualified production hardware; the AArch64-QEMU Silo remains `DEV_REFERENCE` only. This decision records why that production-hardware prerequisite remains unsatisfied and does not weaken either ADR.
...
- [ADR-0008](./0008-protected-relay-tls-endpoint-ownership.md) — authority-owned relay TLS still requires an exact qualified production root and does not relax this NO-GO.
