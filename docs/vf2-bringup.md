# ViCell on VisionFive2 — Board Bring-Up Guide

Boot ViCell on a real StarFive VisionFive2 (JH7110, RV64GC) and reach an interactive `ViCell>` shell via UART serial. No SD card driver required — all cells are embedded in the kernel binary.
...
2. Compiles `vicell-kernel` with `--features board-vf2 --release`
...
[ViCell kernel]
ViCell v0.2.0 — RISC-V 64 — Cellular SAS
...
The VisionFive2 JH7110 SoC uses the same PLIC (`0x0C000000`), UART NS16550 (`0x10000000`), and CLINT (`0x02000000`) base addresses as the QEMU RISC-V virt machine. No peripheral address changes were needed in ViCell to support this board.
