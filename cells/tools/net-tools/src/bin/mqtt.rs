...
pub fn main() {
    ...
    let subcmd = match parts.next() {
        Some(s) => s,
        None => {
            println("mqtt: missing subcommand");
            return;
        }
    };
    let hostport = match parts.next() {
        Some(s) => s,
        None => {
            println("mqtt: missing host:port");
            return;
        }
    };
    ...
    let (host, port) = match hostport.rfind(':') {
        Some(i) => (
            &hostport[..i],
            parse_u16(&hostport[i + 1..]).unwrap_or(1883),
        ),
    ...
    let cap_id = match sys_recv(0, &mut resp_buf) {
        ...
        _ => {
            println("mqtt: TcpConnect syscall failed");
            return;
        }
    };
    println("connected");
    ...
    match subcmd {
        ...
        "subscribe" => {
            do_subscribe(&mut conn, topic);
        }
        _ => {
            println("mqtt: unknown subcommand");
        }
    }
    close_socket(cap_id, net_ep);
}
...
impl MqttConn {
    ...
    fn take_packet(&mut self, out: &mut [u8; 256]) -> Option<(u8, usize)> {
        ...
        if self.len < total || total > out.len() {
            return None;
        }
        let ctrl = self.buf[0];
    ...
    fn recv_packet(&mut self, out: &mut [u8; 256], max_polls: usize) -> Option<(u8, usize)> {
        for _ in 0..max_polls {
            if let Some(pkt) = self.take_packet(out) {
                return Some(pkt);
            }
            if self.fill() == 0 {
                sys_yield();
...
fn do_publish(cap: u32, topic: &str, payload: &str, net_ep: usize) {
    ...
    if tb.len() > 64 {
        println("mqtt: topic too long (max 64 bytes)");
        return;
    }
    if pb.len() > 256 {
        println("mqtt: payload too long (max 256 bytes)");
        return;
    }
...
fn do_subscribe(conn: &mut MqttConn, topic: &str) {
    ...
    if !subscribed {
        println("mqtt: SUBACK not received");
        return;
    }
    println("subscribed");
    ...
    for _ in 0..10_000usize {
        ...
        let (_, rl_bytes) = match decode_remaining_len(&buf[..n]) {
            Some(v) => v,
            None => continue,
        };
        let vh = 1 + rl_bytes;
