...
impl<'a> ViStateTransfer for ShellState<'a> {
    ...
    fn serialize_state(&self, buf: &mut [u8]) -> ViResult<usize> {
        ...
        buf[pos..pos + 4].copy_from_slice(&hist_count.to_le_bytes());
        pos += 4;
        for i in 0..self.history.len() {
            if let Some(entry) = self.history.get(i) {
                let el = entry.len() as u16;
                buf[pos..pos + 2].copy_from_slice(&el.to_le_bytes());
                pos += 2;
                buf[pos..pos + entry.len()].copy_from_slice(entry.as_bytes());
                pos += entry.len();
            }
        }
        ...
        buf[pos..pos + 4].copy_from_slice(&alias_count.to_le_bytes());
        pos += 4;
...
impl ViStateTransfer for ShellStateOwned {
    fn state_size(&self) -> usize {
        0
    } // not used for deserialise-only path
    ...
    fn deserialize_state(&mut self, buf: &[u8]) -> ViResult<()> {
        ...
        for _ in 0..hist_count {
            ...
            let el = u16::from_le_bytes([buf[pos], buf[pos + 1]]) as usize;
            pos += 2;
            if pos + el > buf.len() {
                return Err(ViError::InvalidInput);
            }
