...
fn cell_main() {
    if ostd::args().iter().any(|arg| arg == PINNED_WORKER_ARG) {
        println("[periph-demo] pinned-poll worker active on hart 0");
        let mut message = [0u8; 8];
        let _ = sys_recv_timeout(0, &mut message, 10);
        return;
    }
    ...
    let bcm_gpio_handled = bcm_gpio_gate::run();
    if !bcm_gpio_handled {
        let gpio_result = {
        ...
        match gpio_result {
            ...
            Err(_) => {
                println("[phase03-gpio] FAIL: BCM and PL061 GPIO unavailable");
                return;
            }
        }
    }
    ...
    if !ostd::set_spawn_argv(&[alloc::string::String::from(PINNED_WORKER_ARG)]) {
        println("[periph-demo] spawn_pinned argv staging failed");
        return;
    }
