...
impl State {
    fn clear(&self) -> bool {
        self.transient == 0 && self.service_handle == 0 && !self.exclusive
    }
}
...
impl NamespaceLedger {
    ...
    pub fn acquire_transient(&self, key: &NamespaceKey) -> Acquire<Transient> {
        self.acquire_shared(key, SharedKind::Transient)
            .map(Transient)
    }

    pub fn acquire_service_handle(&self, key: &NamespaceKey) -> Acquire<ServiceHandle> {
        self.acquire_shared(key, SharedKind::ServiceHandle)
            .map(ServiceHandle)
    }
    ...
    pub fn reserve_two(
        ...
        if entries.get(&first).is_some_and(|s| !s.clear())
            || second
                .as_ref()
                .is_some_and(|k| entries.get(k).is_some_and(|s| !s.clear()))
        {
            return Err(AcquireError::Conflict);
        }
        entries.entry(first.clone()).or_default().exclusive = true;
        if let Some(key) = &second {
            entries.entry(key.clone()).or_default().exclusive = true;
        }
        drop(entries);
...
impl Drop for SharedLease {
    fn drop(&mut self) {
        ...
        let state = entries
            .get_mut(&key)
            .expect("lease entry must exist during drop");
    ...
    fn drop(&mut self) {
        ...
        let mut release = |opt: &mut Option<NamespaceKey>| {
            if let Some(key) = opt.take() {
                let state = entries
                    .get_mut(&key)
                    .expect("reservation entry missing on drop");
