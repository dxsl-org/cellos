...
/// and `yield_once` yields after rejection. Returns `Err(())` when the clock is
/// unavailable or admission exhausts the budget.
pub fn admit_request_until_deadline(
    ...
    loop {
        ...
        let elapsed = now_ticks().ok_or(())?.wrapping_sub(started_at);
...
/// Returns `Ok(None)` when cancellation wins and `Err(())` on deadline or
/// clock/receive failure.
pub fn receive_until_deadline(
    ...
    mut cancelled: impl FnMut() -> bool,
    mut now_ticks: impl FnMut() -> Option<u64>,
    mut recv_slice: impl FnMut(u64) -> Result<Option<usize>, ()>,
) -> Result<Option<usize>, ()> {
    loop {
        if cancelled() {
            return Ok(None);
        ...
        match recv_slice(remaining.min(poll_ticks))? {
            Some(sender) => return Ok(Some(sender)),
            None => {}
        }
    }
}
