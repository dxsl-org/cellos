...
// reason: implements the P04/P05 STUN reflexive-address client
// (query_reflexive_addr) for NAT traversal in the net-broker robot-swarm
// feature; not yet called from main.rs's dispatch loop or BrokerIdentity —
// the doc comment on `identity::reflexive_addr` names this module as the
// intended caller, but the wiring hasn't landed.
#![allow(dead_code)]
...
fn parse_stun_response(raw: &[u8], tx_id: &[u8; 12]) -> ViResult<([u8; 4], u16)> {
    ...
    if u16::from_be_bytes([frame[0], frame[1]]) != 0x0101 {
        return Err(ViError::IO);
    }
