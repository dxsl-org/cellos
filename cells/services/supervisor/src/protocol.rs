...
impl<'a> HotswapRequest<'a> {
    ...
    pub fn parse(buf: &'a [u8]) -> Option<Self> {
        if buf.len() < 1 + SVC_NAME_LEN + ELF_PATH_LEN { return None; }
        if buf[0] != OP_HOTSWAP { return None; }
        // SAFETY: slices are within bounds and correctly sized.
        let target_service = buf[1..1 + SVC_NAME_LEN].try_into().ok()?;
        let new_elf        = buf[1 + SVC_NAME_LEN..1 + SVC_NAME_LEN + ELF_PATH_LEN].try_into().ok()?;
        Some(Self { target_service, new_elf })
    }

    /// Return the null-terminated service name as a &str (up to first NUL).
    pub fn service_name(&self) -> &str {
        let end = self.target_service.iter().position(|&b| b == 0)
    ...
    pub fn elf_path(&self) -> &str {
        let end = self.new_elf.iter().position(|&b| b == 0)
