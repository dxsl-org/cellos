    ...
    F: FnMut(&[DescBuf]) -> u32,
{
    ...
    if crate::virtqueue_guard::pending_count(q_size, *last_avail_idx, avail_idx).is_none() {
        // A conforming driver cannot make more than q_size unpublished entries
        // available. Resync instead of allowing a guest-controlled CPU loop.
    ...
    while *last_avail_idx != avail_idx {
        ...
        *last_avail_idx = last_avail_idx.wrapping_add(1);
        ...
        let mut cur = head;
        let mut valid = crate::virtqueue_guard::valid_descriptor(head, q_size);
        ...
        let written = if valid && terminated {
            handle(&bufs)
        } else {
            0
        };
        ...
        if crate::vmm::write_guest_memory(vm_id, qcfg.used_gpa + elem_off as u64, &elem) != 8 {
            break;
        }
