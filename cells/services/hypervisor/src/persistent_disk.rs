...
pub type PersistentDisk = (
    usize,
    api::vfs_file_handles::ViVfsFileHandle,
    u64,
);
...
pub fn open() -> Option<PersistentDisk> {
    ...
    let root = api::ipc::VfsRequest::OpenRootDir { path: "/mnt" };
    let api::ipc::VfsResponse::DirHandle(mnt) =
        ostd::ipc::service_call_typed(vfs_tid, &root, &mut request, &mut response).ok()?
    else {
        return None;
    };
    ...
    match ostd::ipc::service_call_typed(vfs_tid, &stat, &mut request, &mut response).ok()? {
        api::ipc::VfsResponse::Stat { size, is_dir: false } => Some((vfs_tid, file, size)),
        _ => None,
    }
}
