...
fn test_handlers_cross_owner_tcp_and_udp_rejection() {
    ...
    let owner_a = SocketOwner { cell_id: 10, generation: 1 };
    let owner_b = SocketOwner { cell_id: 20, generation: 1 };
    ...
    let close_req = NetRequest::TcpClose { cap_id: cap_id as u32 };
    ...
    let udp_h = sockets.add(smoltcp_udp::Socket::new(
        smoltcp_udp::PacketBuffer::new(alloc::vec![smoltcp_udp::PacketMetadata::EMPTY; 2], alloc::vec![0u8; 128]),
        smoltcp_udp::PacketBuffer::new(alloc::vec![smoltcp_udp::PacketMetadata::EMPTY; 2], alloc::vec![0u8; 128]),
    ));
    ...
    assert_ne!(table.get_state(udp_cap, owner_a), Some(SocketState::Listening));
