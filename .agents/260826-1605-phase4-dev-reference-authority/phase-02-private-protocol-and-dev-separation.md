...
- Profile validation is two-gated: frame authentication first, then serialized `AuthorityState` admission persists identity/boot/sequence and exact upload-state checks before an opaque token can invoke bank, X.509, or TPM work. Authenticated stale/replayed requests perform none of that work; exact idempotent retries require a newer sequence durably persisted before response.
...
- 2026-08-31 — Correction: Phase 2's wire/state evidence remained valid, but the cross-crate Phase 4 fixture had acknowledged `Uploading` before bank initialization. The Phase 4-owned thin adapter now orders media initialization/readback and chunk write/readback before protected progress acknowledgement, permits initialization only from the pre-authorize `Pending` state, authenticates the existing bank on every `Uploading` retry, and reboots at every injected boundary without media/protected-state inversion. Absent/corrupt retry headers and corrupt committed prefixes seal. Phase 2 retains sole wire/state ownership; Phase 4 owns this media adapter and its 14/14 validator integration tests.
