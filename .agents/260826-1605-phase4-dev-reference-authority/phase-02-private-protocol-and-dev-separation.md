...
- 2026-08-29 — Result: the Phase 4 journal consumer exposed a missing adapter-verification seam. Phase 2 now owns an immutable protected-record binding view and exact successor verifier for absorbing seal, boot, signed-time challenge/lease, relay, identity, floor, and revision-exhaustion edges. Canonical v1 bytes, operations, transitions, and public KMS fixtures are unchanged; the complete authority-protocol suite passes 28 host tests, including proof that unauthenticated profile requests never invoke the root verifier.
