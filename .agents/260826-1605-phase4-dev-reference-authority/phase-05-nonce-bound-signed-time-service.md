...
- Hand off the exact immutable marker set `DEV_REFERENCE`, `SOFTWARE_HARNESS`, `AWS_DEV_SIGNED_TIME`, `aws-dev-signed-time`, and `cellos-dev-time-v1` to Phase 2's production checker through the reviewed manifest contract; this phase never modifies the checker.
...
- 2026-08-30 — Handler-orchestration `SOFTWARE_HARNESS` slice complete: `SignedTimeHandler` now enforces canonical parse → transactional active registration/state snapshot → registration-gated receipt recovery before any clock dependency. Exact retries re-sign and canonically encode the immutable committed response without loading the protected floor, sampling time, allocating, writing, or refreshing expiry. Only canonical receipt absence loads one exact uint64 protected floor, requests one authenticated admitted sample for that floor, commits once, signs once, and encodes once. Every malformed input/output or dependency failure stops at its boundary with one value-free unchained error and no retry, fallback, cache, holdover, alternate source, or logging. Dependencies remain injected; no concrete authenticated clock adapter, protected-floor implementation, Lambda event adapter, boto3 construction, manifest instance, credential, network call, AWS resource, or deployment was added. Focused and full tests pass 209/209 and final review found no remaining scoped issue.
- 2026-08-31 — CloudFormation/IAM `SOFTWARE_HARNESS` slice complete: a parameterized JSON-syntax CloudFormation template now freezes the intended retained DynamoDB table, retained P-256 signing key and control chain, exact runtime data/sign/log permissions, enforced AWS Signer profile-version code signing, retained function/version, alias-only `POST /v1/time` ingress, bounded logs, dependency alarms, and separated deploy/code-sign-approval/IAM-review/key-policy roles under an externally supplied boundary. Static tests enforce resource lifecycle, KMS algorithm/policy scope, alias/API bindings, platform-created log-stream permissions, pairwise principal separation, per-action CloudFormation IAM semantics, and absence of administrative `kms:Sign`. The template explicitly declares `Deployable: false` and `LiveIsolationClaim: false`: a mutable same-stack graph cannot prove indirect signing isolation, validate external boundary/SCP contents, prove the actual CloudFormation service role, approve an artifact, compose the missing provider/runtime entrypoint, or replace operator negative probes. The deliberate KMS lockout-safety bypass is coupled to a retained exact key administrator and is not deployment authorization. No credential, AWS call, resource creation, network action, packaging, or deployment occurred. Infrastructure tests pass 18/18, the full suite passes 227/227, and final security review found no remaining scoped issue.
...
- 2026-08-31 — Production-rejection handoff review complete without modifying the Phase 2-owned checker: the reviewed manifest module exports one immutable exact marker set covering `DEV_REFERENCE`, `SOFTWARE_HARNESS`, both AWS DEV signed-time names, and `cellos-dev-time-v1`; a focused test freezes that contract. The existing checker already rejects the other handed-off markers in feature names, artifact bytes, and split-chunk boundaries and keeps clean candidates blocked by ADR-0006; its 7/7 tests pass. Phase 2 still owns adding `SOFTWARE_HARNESS` to the checker matrix. The authenticated clock adapter is explicitly blocked on the missing custom provider contract, but that blocker no longer stops independent software work. The signed-time suite passes 231/231.
