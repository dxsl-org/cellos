...
- [x] Authenticate and durably state-admit profile begin/chunk/validation requests before bank, policy, or TPM work; bind the journal-selected inactive slot and state-derived CSR handle; 39 authority-protocol host tests pass.
...
- [x] Host model rejects TLS signing from `Empty`, `Pending`, `Uploading`,
  ...
  both mandatory; either mismatch seals. It also proves a staged receipt can
  transition to `ReceiptConsumed` exactly once and seals a newer-sequence replay.
