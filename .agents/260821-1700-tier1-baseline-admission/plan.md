...
| 04 | [Run hostile evidence, approvals, and ledger closure](phase-04-negative-evidence-and-approval.md) | blocked | 3d | 03 | no |
...
4. **Release gate (before ledger PASS):** Phase 04 hostile suite passes; security owner signs the threat/evidence package; a reviewer independent of the implementer signs the review; Phase 02 records content-addressed PASS evidence. Any anchor, policy, provenance format, floor backend, recovery algorithm, or loader-gate change invalidates approval and requires rerun.

Current non-waivable blockers: there is no qualified non-replayable external floor or physical replay/power-loss qualification package; no approved owner-store, provenance, or floor parser/persistence path; no provisioned owner anchor; no provisioned publisher anchor; no production profile or controlled CI-to-final-ELF provenance record; no common loader/task-creation runtime wiring and no-task-on-denial proof; no required human approvals; and no Phase 04 hostile/release evidence or governed ledger PASS. The completed Core+harness-only resolution is recorded in [`phase-03-core-harness-blocker-resolution.md`](phase-03-core-harness-blocker-resolution.md); it satisfies none of those physical, integration, governance, or human gates. Therefore production admission MUST remain disabled and the Phase 02 ledger row MUST remain BLOCKED.
