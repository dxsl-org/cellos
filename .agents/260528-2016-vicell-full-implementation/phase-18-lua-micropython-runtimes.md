...
- **MicroPython 1.24.1**: COMPLETE — C runtime compiled for RISC-V 64 bare-metal via `cc` crate; `gen_genhdr.py` (header generator), `mpconfigport.h` (port config), `mphalport.c` (HAL hooks), `micropython.ld` (cell linker script), `main.rs` (REPL + script driver) all implemented; 547 KB release binary integrated into `kernel_fs.img`
...
cells/runtimes/lua/
  ...
  ├─ glue/lua_ViCell_glue.c       (from Phase 10, extended)
...
cells/runtimes/micropython/
  ...
  ├─ glue/upy_ViCell_port/         ── port adapter (mphalport.c, mpconfigport.h)
  ├─ src/main.rs
  ├─ src/repl_session.rs
  └─ src/bindings_io.rs
...
- `cells/runtimes/lua/glue/lua_ViCell_glue.c` — implement `io_open_ViCell`, `io_read_ViCell`, `io_write_ViCell`, `os_execute_ViCell`
...
- `cells/runtimes/lua/src/bindings_io.rs` — Rust thunks `extern "C" fn ViCell_fopen(path)` → calls into `libs/ostd::File`
- `cells/runtimes/micropython/vendor/micropython-1.24.1/` — vendored source (similar to Phase 10's Lua vendoring)
- `cells/runtimes/micropython/glue/upy_ViCell_port/mpconfigport.h` — MicroPython port config (disable threads, fileio routes via VFS shim)
- `cells/runtimes/micropython/glue/upy_ViCell_port/mphalport.c` — HAL hooks: `mp_hal_stdout_tx_strn`, `mp_hal_stdin_rx_chr`
...
- `docs/scripting-guide.md` — how to write Lua/Python scripts on ViCell
...
4. Extend `lua_ViCell_glue.c` with VFS-backed `lua_io_open`, etc.
   - Implement `static FILE* ViCell_fopen(const char* path, const char* mode)` calling Rust thunk
   - Implement `static size_t ViCell_fread/fwrite/fclose/fseek/ftell`
   - Register in glue so Lua's `io.open` etc. route here
5. Rust FFI in `bindings_io.rs`:
   ```rust
   #[no_mangle] pub extern "C" fn ViCell_fopen_rs(path: *const u8, len: usize, mode: u8) -> *mut VFile { … }
   ```
...
14. `bindings_io.rs`: `ViCell_open`, `ViCell_read`, `ViCell_write` for `open()`/`os.system()`
...
- [ ] Extend `lua_ViCell_glue.c` with VFS-backed io.open, etc.
