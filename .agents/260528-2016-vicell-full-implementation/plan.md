---
title: "ViCell Complete Implementation Roadmap (v0.2.1 → v1.0.0)"
description: "23-phase plan covering kernel fixes, multi-arch HAL, services, runtimes, security, CI/CD, and community infra for ViCell v1.0 release."
status: pending
priority: P1
effort: 2180h
branch: main
tags: [kernel, hal, infra, feature, security, ViCell, ipc, vfs, network, compositor, runtime]
created: 2026-05-28
---

# ViCell Complete Implementation Roadmap
...
**🎉 CORE BOOT MILESTONE ACHIEVED**: ViCell boots to shell prompt on 128MB RAM!
...
- **Fully working**: Boot chain → OpenSBI → kernel → init → VFS → Config → Input → Shell prompt (`ViCell >` displays) 
...
- **Phase 17** (Enhanced Shell) — Parser, executor, readline, history, jobs, aliases all exist as .rs files; shell boots to `ViCell >` prompt
...
- **Boot verification:** Plan.md baseline says "ViCell boots to `ViCell >` shell prompt" — consistent with Phase 17 near-complete → complete status
...
- **Port Config (`mpconfigport.h`)**
  - REPR_A tagged pointer support (ViCell SAS-aware)
...
- **Integration Stubs (`ViCell_stubs.c`)**
...
- **Linker Script (`micropython.ld`)**
  ...
  - Matches ViCell cell ABI expectations
...
- [x] mpconfigport.h ViCell-specific port config written
- [x] mphalport.c HAL hooks implemented (I/O + readline integration)
- [x] ViCell_stubs.c module/function stubs created
...
**Result:** full VirtIO set (block + NIC + keyboard + GPU) boots to `ViCell >`. `run.ps1` re-attaches `-device virtio-gpu-device` by default. New test `gpu_framebuffer_initialises` asserts framebuffer setup + shell reached. Phase 16 → **complete**. Integration suite now **7/7 green**.
...
3. **[Architecture]** Phase 07 FileHandle — single-owner-moveable vs shared Arc?
   ...
   - **Answer:** Single-owner moveable — matches ViCell LBI model, simpler reasoning
...
| 17 | Shell with interactive REPL + 20+ built-in commands | Shell spawns + prints "ViCell >" prompt; 20+ commands in builtins.rs | Interactive serial I/O NOT consumed by shell (test `shell_executes_echo` is #[ignore]'d); command parsing works but I/O loop unverified in QEMU | ⚠ partial |
...
2. **Null-heap fault (`addr=0x8`)** — picolibc's allocator (including the
   ...
   8 MB heap arena in the glue (`lua_ViCell_glue.c`). With a real heap, picolibc's
   own malloc/realloc/free run unmodified — no allocator reimplementation.

#### Changes

- `cells/runtimes/lua/glue/lua_ViCell_glue.c` — replaced the arena `lua_Alloc` +
...
- `cells/runtimes/lua/src/main.rs` / `ffi.rs` — reverted to `luaL_newstate`
  (default allocator now works); removed the obsolete `lua_newstate`/`ViCell_lua_alloc` FFI.
