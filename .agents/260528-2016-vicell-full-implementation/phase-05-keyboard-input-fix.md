...
| Re-armed buffer overruns if events arrive faster than drain | Low | Med | Size the input virtqueue to ≥ 64 entries; bound shell ring to 256; drop oldest on overflow with `[ViCell][input] dropped event` log |
