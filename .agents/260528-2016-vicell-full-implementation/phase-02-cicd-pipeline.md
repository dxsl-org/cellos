...
- QEMU boot test must time-out (`timeout 60s qemu …`) and `grep` serial output for a known banner — kernel must print a stable string early (e.g. `[ViCell] kernel boot`).
...
**Modify:**
- `kernel/src/main.rs` — ensure early `println!("[ViCell] kernel boot v{}…")` is the first serial output (CI greps for it)
...
1. Verify / create `rust-toolchain.toml`:
   ...
   profile = "minimal"
   ```
2. Locate stable boot banner string in `kernel/src/main.rs`. If absent, add `println!("[ViCell] kernel boot v{}", env!("CARGO_PKG_VERSION"))` as the first console output after UART init.
3. Create `scripts/qemu-boot-test.sh`:
   ...
   timeout 60 qemu-system-riscv64 -machine virt -nographic -bios default \
     -kernel "$KERNEL" 2>&1 | tee qemu.log | grep -q "\[ViCell\] kernel boot" || {
...
- [x] Add stable boot banner in `kernel/src/main.rs` (idempotent) — `[ViCell] kernel boot v<ver>` as first UART output
...
- `qemu-boot` job sees `[ViCell] kernel boot` within 60s
