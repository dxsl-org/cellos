...
- Pipes between cells in ViCell aren't POSIX file descriptors — they are owned-buffer IPC channels (Phase 07 capability + buffer transfer). Shell builds a chain: A's stdout cap is wired to B's stdin cap before spawn.
...
- Line editing needs proper key handling — depends on Phase 14's KeyEvent (not raw bytes). Implement minimal readline: arrows for history, Home/End for line nav, Ctrl+A/E shortcuts, Backspace/Delete, history file persisted to `~/.ViCell_history` via VFS.
...
- `cells/apps/shell/src/aliases.rs` — alias table (read from `~/.ViCellrc`)
...
5. **History** `cells/apps/shell/src/history.rs`:
   ...
   - On boot: read `~/.ViCell_history` line by line
...
9. **Aliases** `cells/apps/shell/src/aliases.rs`:
   - On boot read `~/.ViCellrc`; lines `alias ll='ls -la'`
...
- History persists across reboot (`~/.ViCell_history`)
