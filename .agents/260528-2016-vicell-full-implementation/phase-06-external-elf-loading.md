...
| init cell now strictly depends on disk → fails on RAM-only QEMU runs | Low | Med | Keep `-DViCell.disk=ram` env that falls back to embedded ELFs (debug builds only) |
