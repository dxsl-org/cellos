...
The `cells/runtimes/lua` cell currently stubs Lua FFI. Link the real Lua 5.4 C source (vendored) into the cell via the `cc` crate, exposing a usable Lua interpreter that runs `.lua` scripts on ViCell. Provides validation of OSTD's libc shim and unlocks Phase 18 (full runtime enhancement).
...
- Patching: Lua expects `LUAI_THROW`, `lua_writestring`, `lua_writeline`, `lua_writestringerror` to be defined. Provide via a small `lua_ViCell_glue.c` rather than patching pristine Lua sources.
...
│   └── README
├── glue/
│   ├── lua_ViCell_glue.c    ← LUAI_THROW, lua_writestring etc.
│   ├── lua_ViCell_setjmp.S  ← per-arch setjmp/longjmp asm
│   └── lua_ViCell_config.h  ← LUA_USE_C89 etc.
...
- `cells/runtimes/lua/glue/lua_ViCell_glue.c` — overrides for missing libc bits + Lua hooks
- `cells/runtimes/lua/glue/lua_ViCell_setjmp.S` — minimal setjmp/longjmp for each arch (RV64 + AArch64 + x86_64)
- `cells/runtimes/lua/glue/lua_ViCell_config.h` — Lua config header defining LUA_USE_C89, custom LUAI_THROW
...
2. **Create `glue/lua_ViCell_config.h`**:
   ```c
   #pragma once
   #define LUA_USE_C89
   ...
   void ViCell_write(const char* s, size_t n);
   #define lua_writestring(s,l)    ViCell_write((s), (l))
   #define lua_writeline()         ViCell_write("\n", 1)
   #define lua_writestringerror(s,p)  ViCell_write((s), strlen(s))
   ```
3. **Create `glue/lua_ViCell_glue.c`**:
   - Define `ViCell_write` calling `ostd::console::write_bytes` via an `extern "Rust"` thunk
   - Provide any `<stdio.h>` shims Lua libs need (fprintf to stderr, etc.)
   - Provide `abort()` calling `ostd::task::exit(1)`
4. **Create per-arch setjmp/longjmp** (`glue/lua_ViCell_setjmp.S`):
...
5. **Write `build.rs`**:
   ...
   fn main() {
       ...
       b.file("glue/lua_ViCell_glue.c");
       b.file("glue/lua_ViCell_setjmp.S");
       b.compile("lua54");
   }
   ```
...
8. **Smoke test**:
   ...
   - `tests/integration/lua_smoke.rs` boots ViCell, drives shell to run lua, captures `4`
...
- [ ] Create `lua_ViCell_config.h`
- [ ] Create `lua_ViCell_glue.c` (write/abort/stdio shims)
- [ ] Create per-arch `lua_ViCell_setjmp.S` (RV64, AArch64, x86_64)
