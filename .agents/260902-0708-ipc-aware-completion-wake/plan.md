...
**Amendment 2026-09-26 — the whole-run requirement moved off the drain onto the wait's own burn.** The drain cannot causally attribute the recordless return (as the evidence note says), and measurement showed it tracks the *caller's* cadence: 26 local observations burned 435 709–1 136 235 ticks with a single one below the ceiling, while all 71 cycles on the runner were ≥ 1.2 M with a real tick spacing of ~1.2–1.4 M mtime (local ~1.03–1.14 M) — a 90 ms band is narrower than the caller cadence of any slow host, so a correct tree failed on CI and passed locally by phase luck. `idle_ipc_wake` now carries `return_ticks` (the wait's own burn, read at the recordless return) and `sender`; a sub-ceiling PASS must still be strictly below the ceiling; and an all-quantum population is accepted as `verdict=quantum-spanning-idle-out` with its measurements printed. The mechanism claim is unchanged and still owned by the deterministic `IPC-PENDING` / `NET-RX-RESERVATION` selftests the same gate requires.
