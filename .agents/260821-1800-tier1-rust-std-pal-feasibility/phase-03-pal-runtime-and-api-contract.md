...
- Unsupported APIs must return the documented `std::io::ErrorKind::Unsupported`/equivalent or abort only where the contract requires divergence; no success-shaped stubs. PAL-019 now has a governed no-`dev-weak-rng` release tuple and zero/error QEMU companion; PAL-031 has bounded caller-owned writable validation and focused hostile direct-opcode evidence. Both remain Deferred pending named approval of the governed manifest.
...
3. Build an API family table with `Supported|Unsupported|Deferred`, exact observable behavior, capability source, and whether availability is required for the initial profile. Keep `PAL-019` and `PAL-031` Deferred while named approval reviews their completed technical backing and evidence.
...
- [x] `PAL-019` production zero/error backing/evidence and `PAL-031` caller-owned buffer backing/evidence are complete; both remain Deferred pending named approval of this governed rebind.
...
The contract and its closed security-backing manifest passed all 36/36 security-manifest tamper attacks within the final 33/33 feasibility suite. Reconciliation confirmed the exact six-path kernel inventory and matching digests/links; final independent security review returned PASS with no findings. The governed production release tuple omits `dev-weak-rng`, its no-default QEMU companion proves zero without synthetic success, and focused direct-opcode evidence covers `GetRandom` hostile rejection and final-authorization races. This verifies technical backing only: `PAL-019` and `PAL-031` remain Deferred until named approval; both human approval rows remain `NOT GRANTED`, and implementation authorization remains blocked.

## Security Considerations

Entropy must fail closed: the default `dev-weak-rng` tuple remains development-only and non-qualifying, while the governed production tuple omits it and the no-default QEMU companion proves zero without synthetic success. Writable syscall buffers require bounded, complete caller ownership and writable mapping before dereference; `GetRandom` has this validation and focused hostile direct-opcode evidence. PAL-019 and PAL-031 remain Deferred until named approval of the governed rebind. Filesystem/network/process APIs cannot gain ambient authority; panic output must not leak secrets or addresses beyond existing policy; allocator/TLS state is per-cell and cannot cross authority boundaries.
