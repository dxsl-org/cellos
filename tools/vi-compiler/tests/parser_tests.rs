...
fn button_callback_binding() {
    ...
    assert!(
        !button.callbacks[0].body.is_empty(),
        "callback body should not be empty"
    );
}
...
fn text_binding_has_interpolation() {
    ...
    match &tb.value {
        Expr::Raw(r) => assert!(
            r.text.contains("\\{count}"),
            "interpolation must be in raw text"
        ),
...
fn error_on_unexpected_toplevel() {
    ...
    assert!(
        result.is_err(),
        "unexpected top-level token should be an error"
    );
}
