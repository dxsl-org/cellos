...
from state_reader_support import CONTRACT, EPOCH, FakeClient, fixture, read_result
...
class StateReaderItemTests(unittest.TestCase):
    ...
    def test_malformed_items_and_attribute_values_fail_closed(self):
        ...
        registration_item = valid[1]["Item"]
        state_item = valid[2]["Item"]
        ...
        cases = (
            [valid[0], None, valid[2]],
            [valid[0], {"Item": DictChild(registration_item)}, valid[2]],
            [valid[0], {"Item": malformed_registration}, valid[2]],
            [valid[0], {"Item": extra_registration}, valid[2]],
            [valid[0], {"Item": malformed_registration_av}, valid[2]],
            [valid[0], valid[1], {"Item": None}],
            [valid[0], valid[1], {"Item": DictChild(state_item)}],
            [valid[0], valid[1], {"Item": malformed_state_av}],
        )
    ...
    def test_wrong_record_keys_fail_closed(self):
        ...
        wrong_registration = copy.deepcopy(responses[1]["Item"])
        wrong_registration["pk"] = {"S": "authority#wrong/registration"}
        wrong_state = copy.deepcopy(responses[2]["Item"])
        wrong_state["pk"] = {"S": "source#wrong/state"}
        for changed in (
            [responses[0], {"Item": wrong_registration}, responses[2]],
            [responses[0], responses[1], {"Item": wrong_state}],
        ):
    ...
    def test_configured_source_epoch_must_match_decoded_state(self):
        ...
        valid = read_result()["Responses"]
        substituted = copy.deepcopy(valid[2]["Item"])
        substituted["source_epoch"] = {"N": str(EPOCH + 1)}
        self.assert_failed(
            read_result(responses=[valid[0], valid[1], {"Item": substituted}]),
        )
