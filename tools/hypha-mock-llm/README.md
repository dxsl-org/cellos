...
isolates the *ViCell side* of the LLM path (cell → net → TLS → HTTP → JSON parse) from the real
...
- Speaks **TLS 1.3** with a self-signed **P-256** cert (auto-generated on first run).
  ViCell's net cell uses embedded-tls `UnsecureProvider` — it does **not** verify the server cert
...
Then boot ViCell with **QEMU user-mode (SLIRP) networking** (the default), where the guest reaches
the host at `10.0.2.2`. In the ViCell shell:
...
HTTP request/response, and JSON shape. The ViCell-guest → host hop still needs a boot run to confirm
