...
/// `main` skipped that wrapper: a cell whose `main` loops forever never noticed,
/// but a finite CLI cell (mqtt, wget) returned into `ret` with `ra = 0` → jump
/// to 0 → instruction page fault (scause=0xc, sepc=0) on exit. `_start` calls
/// `main` the same way, so loop-forever cells are unaffected; the init-array
/// loop is a no-op for pure-Rust cells (no static ctors) and correctly runs C
/// static constructors for Tier-1b cells.
///
/// `_start`'s init-array addressing is PC-relative on all three arches (riscv
/// `lla`, aarch64 `adrp`/`add`, x86 RIP-relative `lea`) so it links clean under
/// `-pie` — the ABS64 landmine that once forced `ENTRY(main)` is gone (Spec 17).
