...
fn fmt_uint(tmp: &mut [u8; 24], mut n: u64, base: u64, upper: bool) -> usize {
    ...
    while n > 0 {
        rev[len] = digits[(n % base) as usize];
        n /= base;
        len += 1;
    }
    for i in 0..len {
...
pub(super) fn fmt_float(tmp: &mut [u8; 64], val: f64, prec: usize) -> usize {
    if val.is_nan() {
        tmp[..3].copy_from_slice(b"nan");
        return 3;
    }
    if val.is_infinite() {
        if val > 0.0 {
            tmp[..4].copy_from_slice(b"+inf");
            return 4;
        } else {
            tmp[..4].copy_from_slice(b"-inf");
            return 4;
...
pub(super) unsafe fn emit_padded(
    ...
    if !left {
        for _ in 0..padding {
            emit_byte(out, pos, cap, pad_char);
        }
    }
    for &b in src {
        emit_byte(out, pos, cap, b);
    }
    if left {
        for _ in 0..padding {
            emit_byte(out, pos, cap, b' ');
...
pub(super) unsafe fn vsnprintf_core(
    ...
    loop {
        ...
        if *fmt.add(fi) == b'.' {
            ...
            let mut p = 0usize;
            while *fmt.add(fi) >= b'0' && *fmt.add(fi) <= b'9' {
                p = p * 10 + (*fmt.add(fi) - b'0') as usize;
                fi += 1;
            }
            prec = Some(p);
        }
        ...
        match spec {
            ...
            b'd' | b'i' => {
                let v: i64 = if long_mod == 2 {
                    args.next_arg::<i64>()
                } else {
                    args.next_arg::<i32>() as i64
                };
                let mut tmp = [0u8; 24];
                let l = fmt_int(&mut tmp, v);
                let mut pb = [0u8; 64];
                let pl = apply_int_prec(&mut pb, &tmp[..l], prec);
                emit_padded(
                    out,
                    &mut pos,
                    cap,
            ...
            b'u' => {
                let v: u64 = if long_mod == 2 {
                    args.next_arg::<u64>()
                } else {
                    args.next_arg::<u32>() as u64
                };
                let mut tmp = [0u8; 24];
                let l = fmt_uint(&mut tmp, v, 10, false);
                let mut pb = [0u8; 64];
                let pl = apply_int_prec(&mut pb, &tmp[..l], prec);
                emit_padded(
                    out,
            ...
            b'x' => {
                let v: u64 = if long_mod >= 1 {
                    args.next_arg::<u64>()
                } else {
                    args.next_arg::<u32>() as u64
                };
                let mut tmp = [0u8; 24];
                let l = fmt_uint(&mut tmp, v, 16, false);
                let mut pb = [0u8; 64];
                let pl = apply_int_prec(&mut pb, &tmp[..l], prec);
                emit_padded(
                    out,
            ...
            b'X' => {
                ...
                let mut tmp = [0u8; 24];
                let l = fmt_uint(&mut tmp, v, 16, true);
                let mut pb = [0u8; 64];
                let pl = apply_int_prec(&mut pb, &tmp[..l], prec);
                emit_padded(
                    out,
            ...
            b'o' => {
                ...
                let mut tmp = [0u8; 24];
                let l = fmt_uint(&mut tmp, v, 8, false);
                let mut pb = [0u8; 64];
                let pl = apply_int_prec(&mut pb, &tmp[..l], prec);
                emit_padded(
                    out,
