...
impl WindowChrome {
    ...
    pub fn titlebar_rect(&self, surf_w: u32) -> Rect {
        Rect::new(
            0.0,
            0.0,
            surf_w as f32 - (BTN_SIZE + BTN_MARGIN) * 3.0,
            TITLEBAR_H,
        )
    }
    ...
    pub fn event(&mut self, e: &Event, surf_w: u32) -> Option<WindowEvent> {
        match e {
            Event::MousePress {
                ...
                if self.close_btn_rect(surf_w).contains(*pos) {
                    return Some(WindowEvent::Close);
                }
                if self.max_btn_rect(surf_w).contains(*pos) {
                    return Some(WindowEvent::Maximize);
                }
                if self.min_btn_rect(surf_w).contains(*pos) {
                    return Some(WindowEvent::Minimize);
                }
...
pub(crate) fn translate_input(ev: InputEvent, mouse_pos: Point) -> Option<Event> {
    match ev {
        InputEvent::MouseMove { x, y, .. } => Some(Event::MouseMove {
        ...
        InputEvent::MouseScroll { dy, .. } => Some(Event::Scroll {
            pos: mouse_pos,
            delta_y: dy as f32,
        }),
        InputEvent::Key(k) => {
            ...
            let key = match k.keysym {
                ...
                KeySym::F1 => KeyCode::F(1),
                KeySym::F2 => KeyCode::F(2),
                KeySym::F3 => KeyCode::F(3),
                KeySym::F4 => KeyCode::F(4),
                KeySym::F5 => KeyCode::F(5),
                KeySym::F6 => KeyCode::F(6),
                KeySym::F7 => KeyCode::F(7),
                KeySym::F8 => KeyCode::F(8),
                KeySym::F9 => KeyCode::F(9),
                KeySym::F10 => KeyCode::F(10),
                KeySym::F11 => KeyCode::F(11),
                KeySym::F12 => KeyCode::F(12),
                _ => return None,
            };
            Some(Event::KeyPress {
                key,
...
impl WindowManager {
    ...
    fn dispatch(&mut self, ev: Event) {
        let Some(win) = self.windows.get_mut(self.focused) else {
            return;
        };
        ...
        if let Some(chrome_ev) = win.chrome.event(&ev, w) {
            match chrome_ev {
                ...
                WindowEvent::Minimize | WindowEvent::Maximize => {
                    /* TODO P08+ */
                    return;
                }
            }
        ...
        let content_ev = match &ev {
            Event::MouseMove { pos } => Event::MouseMove {
                pos: Point::new(pos.x, pos.y - TITLEBAR_H),
            },
            ...
            Event::Scroll { pos, delta_y } => Event::Scroll {
                pos: Point::new(pos.x, pos.y - TITLEBAR_H),
                delta_y: *delta_y,
            },
            other => other.clone(),
