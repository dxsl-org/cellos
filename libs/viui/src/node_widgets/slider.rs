...
impl ViNode for Slider {
    ...
    fn paint(&self, cx: &mut RenderCtx<'_>) {
        ...
        for (a, bb) in [
            (
                ...
                Point::new(thumb.x + thumb.w, thumb.y),
            ),
            (
                Point::new(thumb.x + thumb.w, thumb.y),
                Point::new(thumb.x + thumb.w, thumb.y + thumb.h),
            ),
            (
                Point::new(thumb.x + thumb.w, thumb.y + thumb.h),
                Point::new(thumb.x, thumb.y + thumb.h),
            ),
            (
                Point::new(thumb.x, thumb.y + thumb.h),
                Point::new(thumb.x, thumb.y),
    ...
    fn event(&mut self, event: &Event) -> bool {
        ...
        match event {
            ...
            Event::MouseMove { pos } if self.dragging.get() => set_from_pos(*pos),
            Event::MouseRelease { .. } => {
            ...
            Event::TouchMove { pos, finger_id: 0 } if self.dragging.get() => set_from_pos(*pos),
    ...
    fn collect_dirty_handles(&mut self, region: DirtyRegion) -> Vec<SubscriptionHandle> {
        ...
        let h = self.value.subscribe(move || {
