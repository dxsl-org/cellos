...
impl ViNode for CheckBox {
    ...
    fn paint(&self, cx: &mut RenderCtx<'_>) {
        ...
        cx.canvas
            .draw_line(Point::new(x0, y0), Point::new(x1, y0), bc); // top
        cx.canvas
            .draw_line(Point::new(x1, y0), Point::new(x1, y1), bc); // right
        cx.canvas
            .draw_line(Point::new(x1, y1), Point::new(x0, y1), bc); // bottom
        cx.canvas
            .draw_line(Point::new(x0, y1), Point::new(x0, y0), bc); // left
    ...
    fn collect_dirty_handles(&mut self, region: DirtyRegion) -> Vec<SubscriptionHandle> {
        ...
        let h1 = self.checked.subscribe(move || {
            region.borrow_mut().mark(bounds);
        });
        let h2 = self.label.subscribe(move || {
