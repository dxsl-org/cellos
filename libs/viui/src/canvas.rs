...
impl<'fb> FramebufferCanvas<'fb> {
    ...
    fn put_pixel(&mut self, x: i32, y: i32, color: Color) {
        ...
        if (px as i32) < cx0 || (px as i32) >= cx1 {
            return;
        }
        if (py as i32) < cy0 || (py as i32) >= cy1 {
            return;
        }
...
impl<'fb> ViCanvas for FramebufferCanvas<'fb> {
    fn fill_rect(&mut self, rect: Rect, color: Color) {
        ...
        let clipped = match rect.intersect(&clip) {
            Some(r) => r,
            None => return,
        };
    ...
    fn draw_text(&mut self, pos: Point, text: &str, style: TextStyle) {
        ...
        for ch in text.chars() {
            ...
            let code = ch as u32;
            let idx = if (0x20..=0x7E).contains(&code) {
                (code - 0x20) as usize
            } else {
                (b'?' - 0x20) as usize
            };
            ...
            if opaque && fully_in_bounds && fully_in_clip {
                ...
                for row in 0..8i32 {
                    ...
                    if bits == 0 {
                        continue;
                    } // C — empty row skip even in slow path
                    for col in 0..8i32 {
                        if bits & (1u8 << col) != 0 {
    ...
    fn draw_image(&mut self, dest: Rect, pixels: &[u8], src_stride: u32) {
        let clip = self.active_clip();
        let clipped = match dest.intersect(&clip) {
            Some(r) => r,
            None => return,
        };
