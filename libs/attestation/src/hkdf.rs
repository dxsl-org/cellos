...
fn hmac_sha256(key: &[u8], message: &[u8]) -> [u8; HASH_LEN] {
    ...
    assert!(message.len() <= MAX_MESSAGE_LEN, "hmac_sha256: message exceeds crate-internal bound");
...
pub fn expand(prk: &[u8; HASH_LEN], info: &[u8], okm: &mut [u8]) {
    assert!(info.len() <= MAX_INFO_LEN, "hkdf::expand: info exceeds crate-internal bound");
    assert!(okm.len() <= 255 * HASH_LEN, "hkdf::expand: requested length exceeds RFC 5869 bound");
