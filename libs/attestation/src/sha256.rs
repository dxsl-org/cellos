...
//!
//! Duplicated from `kernel/src/sha256.rs` (single audited file, per KISS — this
//! crate must not create a kernel→lib dependency cycle, and the file has no
//! external deps). If a FIPS-correctness fix ever lands in one copy, mirror it in
//! the other. Self-contained, `no_std`, zero-dependency: used here as the digest
//! HKDF (`super::hkdf`) hashes over, and as the pre-hash [`super::token`] signs.
