...
use crate::{
    fs::{
        DirStream, Dirent, FileType, Inode, InodeId, Result, FsError,
...
impl TryFrom<Fat32Attributes> for FileType {
    ...
    fn try_from(value: Fat32Attributes) -> Result<Self> {
        if value.contains(Fat32Attributes::DIRECTORY) {
            ...
            // warn!("Entry is neither a regular file nor a directory. Ignored.");
             // Treat others as files for now or error? 
             // Ideally we should support read only files too which might not have archive bit?
             // FAT spec says: "The Archive bit is set when the file is created or renamed."
             if !value.contains(Fat32Attributes::DIRECTORY) && !value.contains(Fat32Attributes::VOLUME_LABEL) {
                 Ok(FileType::File) // Simplification
             } else {
...
impl<T: Fat32Operations> Fat32DirStream<T> {
    ...
    async fn next_fat32_entry(&mut self) -> Result<Option<Fat32DirEntry>> {
        loop {
            ...
            let file_type = match FileType::try_from(dir_entry.attributes) {
                Ok(t) => t,
                Err(_) => {
            ...
            let attr = FileAttr {
                ...
                file_type,
                mode: FilePermissions::from_bits_truncate(0o755), // Simplified from bits_retain
...
fn is_leap_year(year: u32) -> bool {
    (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0) // Fixed is_multiple_of
}
