...
pub(super) fn run() -> usize {
    ...
    for case in ["AP-01", "AP-02", "AP-03", "AP-04"] {
        record(case, &mut failures, || with_populated_baseline(case, NO_AUDIT_RECORDS, || {
    ...
    record(
        "AP-05",
        &mut failures,
        || with_populated_baseline("AP-05", PLATFORM_SINGLETON_AUDIT_RECORDS, platform_singleton_denial),
    );
    record(
        "AP-06",
        &mut failures,
    ...
    for case in ["AP-07", "AP-08"] {
        record(case, &mut failures, || with_populated_baseline(case, NO_AUDIT_RECORDS, || {
            denied_with_full_rollback(case, || super::super::spawn_trusted_init(crate::INIT_ELF).map(|_| ()))
        }));
    }
    record(
        "AP-09",
        ...
        denied_with_governed_rollback("AP-09", 1, || {
            super::super::spawn_from_path("/bin/vfs", super::super::SpawnRequest::governed_boot())
                .map(|_| ())
        })
    }));
    record(
    ...
    record(
        ...
        denied_with_governed_rollback("AP-11", 2, || {
            let mut request = super::super::SpawnRequest::governed_boot();
            request.replacement = Some(
                crate::cell::hotswap::ReplacementReservation::invalid_for_test(),
            );
            super::spawn_governed_platform(request).map(|_| ())
        })
    }));
