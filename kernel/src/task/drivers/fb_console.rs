...
use log::info;
use crate::task::drivers::virtio_gpu::{GPU_CONTEXT, GpuContext};
...
impl FramebufferConsole {
    ...
    pub fn init() {
        {
            ...
            if let Some(gpu_ctx) = GPU_CONTEXT.lock().as_mut() {
                let mut console = FramebufferConsole::new(gpu_ctx.width, gpu_ctx.height);
    ...
    pub fn write_str(s: &str) {
        ...
        if let Some(console) = console_guard.as_mut() {
            if let Some(gpu) = GPU_CONTEXT.lock().as_mut() {
                // Clear old cursor
                console.draw_rect(gpu, console.cursor_x, console.cursor_y, font::FONT_WIDTH as u32, font::FONT_HEIGHT as u32, console.bg_color);
    ...
    fn draw_cursor(&self, ctx: &mut GpuContext) {
        self.draw_rect(ctx, self.cursor_x, self.cursor_y, font::FONT_WIDTH as u32, font::FONT_HEIGHT as u32, self.fg_color);
    }
