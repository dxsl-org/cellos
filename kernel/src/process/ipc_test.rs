...
pub fn test_ping_pong() {
    ...
    // Spawn Task B (Server)
    let server_id = super::spawn("ipc-server", Vec::new());
    info!("Spawned Server: Task {}", server_id);
    
    // Spawn Task A (Client)
    let client_id = super::spawn("ipc-client", Vec::new());
...
pub fn test_borrow_read() {
    ...
    let lender_id = super::spawn("lender", Vec::new());
    let borrower_id = super::spawn("borrower", Vec::new());
...
pub fn test_multiple_clients() {
    ...
    let _server_id = super::spawn("multi-server", Vec::new());
    
    for i in 0..3 {
        use alloc::string::ToString;
        let name = alloc::string::String::from("client-") + &i.to_string();
        let client_id = super::spawn(&name, Vec::new());
